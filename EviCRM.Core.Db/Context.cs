...
namespace EviCRM.Server.Core.EntityFramework
{
    public class Context : DbContext
    {
        ...
        public bool ChatDialogs_Remove(int dialog_id)
        {
            var chat_dialogs = chat.FirstOrDefault(p => p.idchat == dialog_id);
            Remove(chat_dialogs);
            SaveChanges();
        ...
        public List<schema_chat> ChatDialogs_Get()
        {
            return chat.ToList();
        }

        //public List<schema_chat> ChatDialogs_Get(schema_users User)
        //{
        //    var chat_dialogs = chat.Where(p => p.idchat.Con);
        //    return chat.ToList();
        //}


        public bool ChatDialogs_Create(schema_chat ChatDialogs)
        {
            chat.Add(ChatDialogs);
            SaveChanges();
            return true;
        }

        public bool ChatDialogs_Update(schema_chat ChatDialogs)
        {
            chat.Update(ChatDialogs);
            SaveChanges();
            return true;
        }
        ...
        {
            todo.markdown_todo_markdatetime = DateTime.Now;

            var schema_markdown_todo_list = markdown_todo_list.FirstOrDefault(p => p.idmarkdown_todo_list == todo.idmarkdown_todo_list);

            if (schema_markdown_todo_list != null)
            {
                schema_markdown_todo_list.markdown_todo_checked = todo.markdown_todo_checked;
                schema_markdown_todo_list.markdown_todo_taskid = todo.markdown_todo_taskid;
                schema_markdown_todo_list.markdown_todo_cardid = todo.markdown_todo_cardid;
                todo.markdown_todo_markdatetime = DateTime.Now;
                schema_markdown_todo_list.markdown_todo_name  = todo.markdown_todo_name;
                markdown_todo_list.Update(schema_markdown_todo_list);
                SaveChanges();
        ...
        public List<schema_marks> Marks_Get(schema_users User)
        {
            var list = marks.Where(p => p.user_id == User.id).ToList();
            return list;
        }
...
if (queue.Count > 0)
                {
                    foreach (var task in queue)
                    {
                        ...
                        sttm.task_variable1 = task.task_variable1;
                        sttm.task_id = task.task_id;
                        sttm.task_cmd = task.task_cmd;
                        sttm.task_author = task.task_author;
                        sttm.task_variable2 = task.task_variable2;

                        TaskTrackingMain_Create(sttm);
                    }
                }
            }
        ...
        public schema_users User_Get(string user_id,bool isID)
        {
            schema_users list = new();
            if (int.TryParse(user_id,out int a))
            {
                list = users.FirstOrDefault(p => p.id == a);
            }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<schema_chat>(entity =>
            {
                entity.ToTable("chat")
                .HasKey(p => p.idchat);


                entity.Property(p => p.chat_avatar)
               .HasColumnType("VARCHAR")
               .HasColumnName("chat_avatar")
               .IsRequired()
               .HasDefaultValue("/avatars/users/default_chatdialog.png")
               .HasMaxLength(250);


                entity.Property(p => p.chat_name)
               .HasColumnType("VARCHAR")
               ...
               .HasMaxLength(250);
                ...
                entity.Property(p => p.chat_isPersonal)
               .HasColumnType("TINYINT(1)")
               .HasColumnName("chat_isPersonal")
               .HasDefaultValue(true)
               .IsRequired();

                entity.Property(p => p.chat_created)
               .HasColumnType("DATETIME")
               .HasColumnName("chat_created")
               .HasDefaultValueSql("CURRENT_TIMESTAMP()")
               .IsRequired();

                entity.Property(p => p.chat_penfriend_id)
               .HasColumnType("VARCHAR")
               .HasColumnName("chat_penfriend_id")
               .IsRequired()
               .HasDefaultValue("$null")
               .HasMaxLength(250);

                entity.Property(p => p.chat_isSecret)
               .HasColumnType("TINYINT(1)")
               .HasColumnName("chat_isSecret")
               .HasDefaultValue(false)
               .IsRequired();

                entity.Property(p => p.chat_key)
               .HasColumnType("VARCHAR")
               .HasColumnName("chat_key")
               .HasDefaultValue("null")
               .IsRequired()
               .HasMaxLength(100);

                entity.Property(p => p.chat_cast)
              .HasColumnType("VARCHAR")
              .HasColumnName("chat_cast")
              .HasDefaultValue("$null")
              .IsRequired()
              .HasMaxLength(500);
    ...
    .HasColumnName("thinclient_user");
      ...
      .HasColumnName("task_members_divs");
       ...
       public DbSet<schema_chat> chat { get; set; }
